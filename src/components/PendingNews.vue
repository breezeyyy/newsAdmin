<template>
	<el-main>
        ...
        <span :class="{auditStyle:scope.row.audit==='已审核',noAuditStyle:scope.row.audit ==='未通过'}">{{
		        scope.row.audit
	        }}</span>
			</el-table-column>
        ...
        <el-button type="primary" @click="audit(dialogData._id,'已审核')">审核通过</el-button>
			...
			<span slot="footer" class="dialog-footer" v-show="dialogData.audit==='已审核'">
        <el-button @click="$store.commit('pendingNews/dialogVisible',false)">取 消</el-button>
        <el-button type="primary" @click="audit(dialogData._id,'待审核')">移除已审核</el-button>
      </span>
