<template>
	<el-main>
		<el-table :data="tableData">
			...
			>
				<span
					:class="{auditStyle:scope.row.audit==='已审核'}"
				>{{ scope.row.audit }}</span>
			</el-table-column>
			<el-table-column
				label="操作"
				width="170"
			...
			>
				<el-row>
					<el-button
						type="warning"
						@click="dialogShow(scope.row._id)"
					>详情</el-button>
				</el-row>
			</el-table-column>
		</el-table>
		...
		>
			<div id="dialog_content">
				...
				<p
					id="dialogP"
					v-html="dialogData.detail.content"
				></p>
			</div>
			<span
				slot="footer"
				class="dialog-footer"
				v-show="dialogData.audit==='已审核'"
			>
		        <el-button
			        @click="$store.commit('PendingNews/dialogVisible',false)"
		        >取 消</el-button>
		        <el-button
			        type="primary"
			        @click="audit(dialogData._id,'待审核')"
		        >移除已审核</el-button>
            </span>
		</el-dialog>
	</el-main>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions('pendingNews', ['nopass', 'passAudit', 'listLength']),
			handleClose (done) {
				done()
			},
			dialogShow (id) {
